  ...
  if (impl_->font) {
    ...
    if (pdf_text_width > cx) {
      ...
      for (auto it = text.begin(), end = text.end(); it != end;) {
        ...
        if (c > std::numeric_limits<unsigned short>::max())
          break;  // PoDoFo does not support UTF-32
        auto cw = units(metrics->UnicodeCharWidth(static_cast<unsigned short>(c)));
        if (pdf_text_width + cw > cx) {
          pdf_text = encode(std::string(text.begin(), it) + "\xE2\x80\xA6");
          break;
        }
        pdf_text_width += cw;
        utf8::next(it, end);
